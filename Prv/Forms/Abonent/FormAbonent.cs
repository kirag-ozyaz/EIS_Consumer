...
namespace Prv.Forms.Abonent
{
    public partial class FormAbonent : FormLbr.FormBase
    {
        ...
        private int inFolder = 0; //папка файлов
        ...
        private void FormAbonent_Load(object sender, EventArgs e)
        {
            ...
            this.SelectSqlData(dsPrv, dsPrv.vUserWorkerGroup, true, "where idGroup = 2467 and  Login = '" + Environment.UserName.ToString().ToUpper() + "' ");

            if (dsPrv.vUserWorkerGroup.Count > 0) AccessToSKUEE = 1;

            //this.SelectSqlData(dsPrv, dsPrv.vUserWorkerGroup, true, "where idGroup = 2484 and  Login = '" + Environment.UserName.ToString().ToUpper() + "' ");

            //if (dsPrv.vUserWorkerGroup.Count > 0) AccessToSKUEEdf = 1;
        ...
        private void GetAccessToSKUEE()
        {
            if (AccessToSKUEE < 0)
            {
                toolStripButtonRefOp.Visible = false;
                tsbLogs.Visible = false;

                tabControlMain.TabPages[2].Parent = null;
                tabControlMain.TabPages[4].Parent = null;
                tabControlMain.TabPages[5].Parent = null;

                tabControlObjDoc.TabPages[0].Parent = null;
                tabControlObjDoc.TabPages[1].Parent = null;

                toolBtnDocActAdd.Visible = false;
                toolBtnDocActEdit.Visible = false;
                toolBtnDocActDel.Visible = false;
                toolBtnDocActCopy.Visible = false;

                tssbObjDocActAdd.Visible = false;
                tsbObjAktEdit.Visible = false;
                tsbObjDocDel.Visible = false;
            }
        ...
        private void LoadAbonent(int codeAbonent, int typeAbn)
        {
            ...
            //Файлы
            SelectSqlData(dsPrv, dsPrv.tAbn_Files, true, "where Deleted = 0 and ParentID = 0 and idAbn = " + dsPrv.vAbn.Rows[0]["id"].ToString() + " order by IsGroup desc,ParentId asc ");
            inFolder = 0;
        ...
        private void LoadSelectAbnObj(int idObj)
        {
            ...
            dsPrv.vDocAct.Clear();
            dsPrv.vPointReg.Clear();
            dsPrv.tASOCA_Meter.Clear();
            dsPrv.tASOCA_Values.Clear();
            dsPrv.ASOCA_Values_30.Clear();
        ...
        private void LoadSelectPoint(int p)
        {
            ...
            //ASOCA
            if (objMeterAdgv.RowCount > 0) this.SelectSqlData(dsPrv, dsPrv.tASOCA_Meter, true, " where  idPoint = " + Convert.ToString(objMeterAdgv.CurrentRow.Cells["idPointdgv"].Value)); ;
            // ПЛОМБИРОВКА
            LoadPlombs();
            SumDeb(p);
        }
        ...
        private void dgvDocAct_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right && e.RowIndex >= 0) // если правой кнопкой
            {
                ...
                ///-----------
                if (toolBtnEditTree.Visible == true && (AccessToSKUEE > 0 || AccessToSKUEEdf < 0)) this.contextMenuDocAct.Show(System.Windows.Forms.Cursor.Position);
            }
        }
        // клик мышки по гриду
        ...
        private void dgvCounter_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                if (dgvCounter["Plomb", e.RowIndex].Value.ToString() == "A")
        ...
        private void dgvCounterPlomb_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if ((dgvCounterPlomb.RowCount > 0) && (e.RowIndex >= 0))
            {
                ...
                if ((dgvCounterPlomb["typeplombnameDataGridViewTextBoxColumn", e.RowIndex].Value.ToString() == "Антимагнитная") && (dgvCounterPlomb["dateDemCounterDgvTxtColumn", e.RowIndex].Value == DBNull.Value))
                    e.CellStyle.ForeColor = Color.Blue;
            }
        }
...
;
        ...
        {
            Reports.FormLogs fRep = new Reports.FormLogs(Convert.ToInt32(dsPrv.vAbn.Rows[0]["CodeAbonent"]), GetIdSelectedPoint());
            fRep.SqlSettings = this.SqlSettings;
        ...
        private void bindingSourceCounterPlomb_CurrentChanged(object sender, EventArgs e)
        {
        ...
        {
            int idPoint = GetIdSelectedPoint();
            Legal.Forms.Meter.FormASOCAMeters fAM = new Legal.Forms.Meter.FormASOCAMeters(idPoint);
        ...
        {
            if (dgvAsocaMeters.RowCount > 0) this.SelectSqlData(dsPrv, dsPrv.tASOCA_Values, true, " where  cnt = " + Convert.ToString(dgvAsocaMeters.CurrentRow.Cells["idAMeter"].Value));
        ...
        private void bAsocaValues30_Click(object sender, EventArgs e)
        {
            ...
            if (dgvAsocaMeters.RowCount > 0) this.SelectSqlData(dsPrv, dsPrv.ASOCA_Values_30, true, " where  cnt = " + Convert.ToString(dgvAsocaMeters.CurrentRow.Cells["idAMeter"].Value));
        ...
        private void RefreshFolder()
        {
            if (inFolder == 0)
            {
                SelectSqlData(dsPrv, dsPrv.tAbn_Files, true, "where Deleted = 0 and ParentID = 0 and idAbn = " +idAbn.ToString() + " order by IsGroup desc,ParentId asc ");
                return;
            }

            SelectSqlData(dsPrv, dsPrv.tAbn_Files, true, "where Deleted = 0 and (id = " + parID.ToString() + "  or  ParentID = " + parID.ToString() + " ) order by IsGroup desc,ParentId asc ");
        }
      ...
      /*  private void tsbAbnFiles_Click(object sender, EventArgs e)
        ...
        private string AddFile(string fileName, int? idTemplate = null, byte[] fileData = null, bool saveAndListen = false)
        {
                ...
                || files.ContainsKey(shortFileName)) && saveAndListen)  // если есть такой файл....
            {
                ...
                {
                    Files.FormNewFileName frmNewFileName = new Files.FormNewFileName(shortFileName);
            ...
            if (saveAndListen)
            {
                ...
                var row = dsPrv.tAbn_Files.NewtAbn_FilesRow();
                row.idAbn = idAbn;
                ...
                dsPrv.tAbn_Files.AddtAbn_FilesRow(row);
                foreach (var item in dsPrv.tAbn_Files)
                {
                    item.EndEdit();
                }
                int idFile = InsertSqlDataOneRow(dsPrv, dsPrv.tAbn_Files);
                RefreshFolder();//SelectSqlData(dsAbn.tAbn_Files, true, "where idAbn = " + idAbn);
                bsAbn_Files.Position = bsAbn_Files.Find("id", idFile);
                dsPrv.tAbn_Files.Where(r => r.id == idFile).ToList().ForEach(r => { r.AcceptChanges(); r.SetModified(); });
            }
        ...
        private void OpenFile(bool isEdit = false)
        {
            if (bsAbn_Files.Current != null)
            {
                ...
                DataSetPrv.tAbn_FilesRow r = (DataSetPrv.tAbn_FilesRow)((DataRowView)bsAbn_Files.Current).Row;
        ...
        private void dgvAbnFiles_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvAbnFiles[e.ColumnIndex, e.RowIndex].Value != null)
            {
                ...
                foreach (DataSetPrv.tAbn_FilesRow row in dsPrv.tAbn_Files)
                {
                    row.EndEdit();
                }
                UpdateSqlData(dsPrv, dsPrv.tAbn_Files);
        ...
        private void InsertToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Files.FormAbnFilesSelectFolder fFolder;
            if (inFolder == 0) fFolder = new Files.FormAbnFilesSelectFolder(idAbn, idF);
            else fFolder = new Files.FormAbnFilesSelectFolder(idAbn, parID);
            fFolder.SqlSettings = this.SqlSettings;
            if (fFolder.ShowDialog() == DialogResult.OK)
            {

                if (dsPrv.tAbn_Files.Rows.Count > 0)
                {
        ...
        private void FolderToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (inFolder == 0)
            {
                Files.FormAbnFilesAddEditFolder fFolder = new Files.FormAbnFilesAddEditFolder(idAbn, 0, true);
                fFolder.SqlSettings = this.SqlSettings;
                if (fFolder.ShowDialog() == DialogResult.OK)
                {
                    RefreshFolder();
            ...
            {
                Files.FormAbnFilesAddEditFolder fFolder = new Files.FormAbnFilesAddEditFolder(idAbn, parID, true);
                fFolder.SqlSettings = this.SqlSettings;
                if (fFolder.ShowDialog() == DialogResult.OK)
                {
                    RefreshFolder();
