...
namespace Legal.Forms
{
    ...
    {
        public static ElectricModel electricModel = new ElectricModel();
        ...
        private int AccessToSKUEE = -1;
        private int AccessToSKUEEdf = -1;
        ...
        private void FormAbn_Load(object sender, EventArgs e)
        {
            ...
            this.SelectSqlData(dsAbn, dsAbn.vUserWorkerGroup, true, "where idGroup = 2467  and  Login = '" + Environment.UserName.ToString().ToUpper() + "' ");

            if (dsAbn.vUserWorkerGroup.Count > 0) AccessToSKUEE = 1;


            this.SelectSqlData(dsAbn, dsAbn.vUserWorkerGroup, true, "where idGroup = 2484 and  Login = '" + Environment.UserName.ToString().ToUpper() + "' ");

            if (dsAbn.vUserWorkerGroup.Count > 0) AccessToSKUEEdf = 1;
            ...
            dsAbn.vG_AbnObj.Rows.Clear();//test
        ...
        private void GetAccessToSKUEE()
        {
            if (AccessToSKUEE < 0)
            {
                ObjMeterAlldgv.Visible = false;
                ObjtabControl.TabPages[2].Parent = null;
               // ObjtabControl.TabPages[3].Parent = null;
                if (AccessToSKUEEdf < 0)
                {
                    tabControlAbn.TabPages[4].Parent = null;
                    tabControlAbn.TabPages[4].Parent = null; //все верно... Сначала убирается вкладка Документы. Потом с тогоже места Файлы
                }

            }

            if (AccessToSKUEE < 0 || AccessToSKUEEdf > 0)
            {
                toolStripSplitButtonAdd.Visible = false;
                tsbRenameFile.Visible = false;
                tsbDeleteFile.Visible = false;

                tssbAbnDocEdit.Visible = false;
                tsbAbnDocDel.Visible = false;
        ...
        private void LoadObj()
        {
            ...
            //Документы объектов
            if (AccessToSKUEE < 0) this.SelectSqlData(dsAbn, dsAbn.vAbnObjDoc_List, true, " where idDocType in (498,1239,1246,1238,497,1240) and idAbnObj = " + IDOBJSELECT.ToString() + " Order by DocDate desc");
                else this.SelectSqlData(dsAbn, dsAbn.vAbnObjDoc_List, true, " where idAbnObj = " + IDOBJSELECT.ToString() + " Order by DocDate desc");
       ...
       private void tssbAbnDocEdit_ButtonClick(object sender, EventArgs e)
        ...
        private void LoadMeter()
        {
            if (Convert.ToString(objMeterdgv.CurrentRow.Cells["meterIDDGVTBC"].Value) != "")
            {
              ...
              //  this.SelectSqlData(dsAbn, dsAbn.vG_ObjTrans, true, " where PointID = " + Convert.ToString(objMeterdgv.CurrentRow.Cells["pointIDDGVTBC"].Value));
                this.SelectSqlData(dsAbn, dsAbn.vPointReg, true, " where type = "+((int)Constants.DevicePointType.TransAmperage).ToString()+" and  idPoint = " + Convert.ToString(objMeterdgv.CurrentRow.Cells["pointIDDGVTBC"].Value));
                //Тарифы
                ...
                if (objMeterdgv.RowCount > 0)
                {
                    ...
                    GosPovtextBox.Text = (objMeterdgv.CurrentRow.Cells["DateCheck"].Value == DBNull.Value) ? "" : Convert.ToDateTime(objMeterdgv.CurrentRow.Cells["DateCheck"].Value).ToString("dd.MM.yyyy");
                    tbOwner.Text = Convert.ToString(objMeterdgv.CurrentRow.Cells["MOwner"].Value);
                    tbKtr.Text = Convert.ToString(objMeterdgv.CurrentRow.Cells["TR_K"].Value);
                    tbMInterval.Text = Convert.ToString(objMeterdgv.CurrentRow.Cells["dGVTBCInterval"].Value);
                    txtPointClass.Text = Convert.ToString(objMeterdgv.CurrentRow.Cells["PointClass"].Value);
                }
                else
                {
                    //Дата Счетчика
                ...
                //Тип трансформатора
                //if (objTransdgv.RowCount >0) TransTypetextBox.Text = Convert.ToString(objTransdgv.CurrentRow.Cells["typeTransDGVTBC"].Value);
                if (objTransdgv.RowCount > 0)
                {
                    tbInterval.Text = Convert.ToString(objTransdgv.CurrentRow.Cells["Interval"].Value);
                    tbTransEnd.Text = Convert.ToString(objTransdgv.CurrentRow.Cells["workerEndDataGridViewTextBoxColumn"].Value);
                    tbTransUst.Text = Convert.ToString(objTransdgv.CurrentRow.Cells["workerBeginDataGridViewTextBoxColumn"].Value);
                ...
                bsObjMeterPlomb.Filter = " Type <> " + ((int)Constants.DevicePointType.TransAmperage).ToString();
                bsObjTransPlomb.Filter = " Type = " + ((int)Constants.DevicePointType.TransAmperage).ToString();
                //Замеры
                ...
                //ASOCA
                if (objMeterAdgv.RowCount > 0) this.SelectSqlData(dsAbn, dsAbn.tASOCA_Meter, true, " where  idPoint = " + Convert.ToString(objMeterAdgv.CurrentRow.Cells["idPointdgv"].Value)); ;
            }
        }
        ...
        private void tsbMeterAdd_Click(object sender, EventArgs e)
        {
            if (dGVEFObj.RowCount > 0)
            {
                ...
                if (objMeterdgv.RowCount > 0)
                {
                    ...
                    Meter.FormMeterAddEdit fAdd = new Meter.FormMeterAddEdit(idObj, idPoint, false,true);
                    fAdd.SqlSettings = this.SqlSettings;
                    fAdd.ShowDialog();
        ...
        private void tsbMeterInfo_Click(object sender, EventArgs e)
        {
            if (objMeterdgv.RowCount > 0)
            {
                ...
                Meter.FormMeterAddEdit fAdd = new Meter.FormMeterAddEdit(idObj, idPoint, true,true);
                fAdd.SqlSettings = this.SqlSettings;
        ...
        private void ClearFieldObject()
        {
            ...
            //счетчики ASOCA
            dsAbn.tASOCA_Meter.Rows.Clear();
            dsAbn.tASOCA_Values.Rows.Clear();
            dsAbn.ASOCA_Values_30.Rows.Clear();
        ...
        private void tsbEditTrans_Click(object sender, EventArgs e)
        {
            if (dGVEFObj.RowCount > 0)
            {
                int idObj = Convert.ToInt32(dGVEFObj.CurrentRow.Cells["idObj"].Value);
                ...
                if (objTransdgv.RowCount > 0)
                {
                    int idPoint = Convert.ToInt32(objTransdgv.CurrentRow.Cells["idpointDGVTBC"].Value);
                    Meter.FormMeterAddEdit fAdd = new Meter.FormMeterAddEdit(idObj, idPoint, false, false);
                    fAdd.SqlSettings = this.SqlSettings;
                    fAdd.ShowDialog();
        ...
        private void tsbInfoTrans_Click(object sender, EventArgs e)
        {
            if (objTransdgv.RowCount > 0)
            {
                int idPoint = Convert.ToInt32(objTransdgv.CurrentRow.Cells["idpointDGVTBC"].Value);
        ...
        private void objTransdgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (objTransdgv.RowCount > 0)
            {
                tbInterval.Text = Convert.ToString(objTransdgv.CurrentRow.Cells["Interval"].Value);
        ...
        private void objMeterdgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (objMeterdgv.RowCount > 0)
            {
                //Дата Счетчика
        ...
        private void tSBasoca_Click(object sender, EventArgs e)
        {
            int idPoint = 0;
            idPoint = Convert.ToInt32(objMeterdgv.CurrentRow.Cells["pointIDDGVTBC"].Value);

            if (idPoint > 0)
            {
                Meter.FormASOCAMeters fAM = new Meter.FormASOCAMeters(idPoint);
                fAM.SqlSettings = this.SqlSettings;
                fAM.ShowDialog();
            }
        }

        private void objMeterAdgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            dsAbn.tASOCA_Meter.Clear();

            if (objMeterAdgv.RowCount > 0) this.SelectSqlData(dsAbn, dsAbn.tASOCA_Meter, true, " where  idPoint = " + Convert.ToString(objMeterAdgv.CurrentRow.Cells["idPointdgv"].Value)); ;
        }

        private void dgvAsocaMeters_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            if (dgvAsocaMeters.RowCount > 0) this.SelectSqlData(dsAbn, dsAbn.tASOCA_Values, true, " where  cnt = " + Convert.ToString(dgvAsocaMeters.CurrentRow.Cells["idAMeter"].Value));

        }

        private void bAsocaValues30_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            if (dgvAsocaMeters.RowCount > 0) this.SelectSqlData(dsAbn, dsAbn.ASOCA_Values_30, true, " where  cnt = " + Convert.ToString(dgvAsocaMeters.CurrentRow.Cells["idAMeter"].Value));
            Cursor.Current = Cursors.Default;
        }

        private void tsbAbnDocInfo_Click(object sender, EventArgs e)
        {
            if (dgvAbnDoc.RowCount > 0)
            {
                int IDABN = Convert.ToInt32(AbnMiniDGV.CurrentRow.Cells["AbnidDGVTBC"].Value);
        ...
        private void objMeterAdgv_SelectionChanged(object sender, EventArgs e)
        {
            dsAbn.tASOCA_Meter.Clear();
