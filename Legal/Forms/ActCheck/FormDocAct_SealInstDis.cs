...
namespace Legal.Forms.ActCheck
{
    public partial class FormDocAct_SealInstDis : FormLbr.FormBase
    {
        ...
        {
            SelectSqlData(dsAbnObjAkt1.vWorkerGroup, true, " where idGroup = 306 order by fio", null, true);
        ...
        #region Farit Раскрасим удаленных мастеров

        private void delWorker_CheckedChanged(object sender, EventArgs e)
        {
            if (delWorker.Checked == true)
                SelectSqlData(dsAbnObjAkt1.vWorkerGroup, true, " where idGroup = 306 order by fio", null, true);
            else
        ...
        private void cbWorker_DrawItem(object sender, DrawItemEventArgs e)
        {
            e.DrawBackground();
            ...
            if (e.Index != -1)
            {
                ...
                string text = currIntemDRView.Row["FIO"].ToString();
                int id = Convert.ToInt32(currIntemDRView.Row["ID"]);


                int type = 0;
                foreach (DataRow currRow in dsAbnObjAkt1.vWorkerGroup.Rows)
                {
                    int currId = (int)currRow["ID"];
                    if (currId == id)
                    {
                        if (currRow["DateEnd"] != System.DBNull.Value)
                            type = 1;
                        break;
                    }
                }
   ...
   //
        ...
        {
            using(FormDocAct_FindPlomb f = new ActCheck.FormDocAct_FindPlomb())
            {
                f.SqlSettings = this.SqlSettings;
                f.Owner = this;
                if (f.ShowDialog() == DialogResult.OK)
                {
                    cbWorker.SelectedValue = f.IDWORKER;
                    cbSealType.SelectedValue = f.IDTYPEPLOMB;
                    SelectSqlData(dsAbnObjAkt1, dsAbnObjAkt1.vPointCheckPlombAdd, true, String.Format("where id = {0}", f.IDPLOMB));
                    dsAbnObjAkt1.vPointCheckPlombAdd.AddvPointCheckPlombAddRow(-1, -1, -1, -1, -1, "Выбрать существующую", DateTime.Now, -1);
                    cbSealNumber.SelectedValue = f.IDPLOMB;
                }
            }
        }
