...
namespace Prv.Forms.Reports
{
    public partial class FormActs : FormLbr.FormBase
    {
                ...
                try
                {
                    ...
                    if (!isDetail)
                        if (isOpl)
                            ...
                            if (isNoKo) if (plomb) strCmd = "select * , dbo.fn_GetPlombsOnPointReg_Antimagnetic (idreg,2) as Plomb ,dbo.fn_GetPlombsOnPointReg_Antimagnetic (idreg,1) as PlombA from fn_PrvRepNoKo_Plomb(@d_beg, @d_end) ";
                                         else strCmd = "select * from fn_PrvRepNoKo(@d_beg, @d_end) ";
                            else if (plomb) strCmd = "select * , dbo.fn_GetPlombsOnPointReg_Antimagnetic (idreg,2) as Plomb ,dbo.fn_GetPlombsOnPointReg_Antimagnetic (idreg,1) as PlombA from fn_PrvRepAct_Plomb() ";
                    ...
                    if (!isDetail)
                    {
                        if (plomb) dAdapt.Fill(dsRep, "tPrv_RepAct_Plomb");
                            else dAdapt.Fill(dsRep, "RepAct");
                        ...
                        if (dsRep.RepAct.Rows.Count > 0)
                        {
                          ...
                          if (dsRep.tPrv_RepAct_Plomb.Rows.Count > 0)
                          {
                              // сохраним акт в журнале
                              // найдем максимальный номер документа
                               MaxNumber = 0;
                              ...
                              if (idJournal > 0)
                              {
                                 /* dsRep.RepAct.Rows[0].BeginEdit();
                                  ...
                                  dsRep.tPrv_RepAct.Rows.Clear();
                                  int j = 0;
                                  for (int k = 0; k < dsRep.tPrv_RepAct_Plomb.Rows.Count; k++)
                                  {
                                      DataRow rw = dsRep.tPrv_RepAct.NewRow();
                                      rw["CodeAbonent"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["CodeAbonent"];
                                      rw["Address"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["Address"];
                                      rw["FIO"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["FIO"];
                                      rw["ContFIO"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["ContFIO"];
                                      rw["CodeController"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["CodeController"];
                                      rw["KladrObj"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["KladrObj"];
                                      rw["KladrObjID"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["KladrObjID"];
                                      rw["Raion"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["Raion"];
                                      rw["Street"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["Street"];
                                      rw["CodeStreet"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["CodeStreet"];
                                      rw["House"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["House"];
                                      rw["HousePrefix"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["HousePrefix"];
                                      rw["Apartment"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["Apartment"];
                                      rw["Mark"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["Mark"];
                                      rw["idMark"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["idMark"];
                                      rw["Number"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["Number"];
                                      rw["CountFloor"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["CountFloor"];
                                      rw["CountPorch"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["CountPorch"];
                                      rw["CountApartment"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["CountApartment"];
                                      rw["TypeObj"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["TypeObj"];
                                      rw["OtherTypeObj"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["OtherTypeObj"];
                                      rw["idReason"] = dsRep.tPrv_RepAct_Plomb.Rows[k]["idReason"];
                                      rw["idJournal"] = idJournal;
                                      rw["Plomb"] = "test";//dsRep.tPrv_RepAct_Plomb.Rows[k]["Plomb"];
        ...
        private void backgroundWorkerNoKo_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (!e.Cancelled)
            {   // если не отмена
                FormRepNoKo rep = new FormRepNoKo(dsRep, MaxNumber.ToString(), DateTime.Now, isDetail);
        ...
        private void bMeterUstDem_Click(object sender, EventArgs e)
        {
            FormActMeterDemUst frm = new FormActMeterDemUst();
            frm.SqlSettings = this.SqlSettings;
            frm.Show();
