...
namespace Legal.Forms.Object
{
    public partial class FormObjAE : FormLbr.FormBase
    {
       ...
       // ElectricModel electricModel = new ElectricModel();
        ...
        private delegate void deligate1();
        private Thread t;
        /// <summary>
        ...
        public FormObjAE(int abn, int idObj, ActionAbnObj actObj, int idObjReg, ActionAbnObjReg actObjReg)
        {
            ...
            dgvLinkSchmAbnObj.Columns["ShemeObjNameAll"].Visible = false;
        
            this.backgroundWorker = new System.ComponentModel.BackgroundWorker();
            this.backgroundWorker.DoWork += new System.ComponentModel.DoWorkEventHandler(this.backgroundWorker_DoWork);
            this.backgroundWorker.RunWorkerCompleted += new System.ComponentModel.RunWorkerCompletedEventHandler(this.backgroundWorker_RunWorkerCompleted);

           // tsbLoadLinkSchm.Enabled = false;
            //t = new Thread(new ThreadStart(LoadSchm));
            //t.Start();
        ...
        private void FormObjAE_Load(object sender, EventArgs e)
        {
            ...
            this.SelectSqlData(dsAbn, dsAbn.vUserWorkerGroup, true, "where idGroup = 2492  and  Login = '" + Environment.UserName.ToString().ToUpper() + "' ");
            if (dsAbn.vUserWorkerGroup.Count > 0) AccessToBinding = 1;
            GetAccessToBinding();
            ...
            // загрузка вышестояшей cетевой организации
            this.SelectSqlDataTableOther(dsAbn, dsAbn.vAbnHighNet, "vAbn", true, "where deleted = 0 and typeAbn = " + ((int)Constants.AbnType.Net).ToString());
            // загрузка котловой организации
            this.SelectSqlDataTableOther(dsAbn, dsAbn.vAbnBoiler, "vAbn", true, "where deleted = 0 and typeAbn = " + ((int)Constants.AbnType.Net).ToString());
...
//            this.SelectSqlDataTableOther(dsLObj, dsLObj.tR_Classifier_Consumer, "tR_Classifier", true, " where ParentKey like ';Consumer;%' and isGroup = 0 and deleted = 0 order by Name");
        ...
        private bool LoadAbnObjReg()
        {
            if (this.SelectSqlData(dsAbn, dsAbn.tAbnObjReg, true, " where id = " + idAbnObjReg.ToString()))
            {
                if (dsAbn.tAbnObjReg.Rows.Count > 0)
                {
                    ...
                    cbAbnBoiler.SelectedValue = dsAbn.tAbnObjReg.Rows[0]["idAbnBoiler"];
                    cbAbnHighNet.SelectedValue = dsAbn.tAbnObjReg.Rows[0]["idAbnHighNet"];
        ...
        private void NewAbnObjReg()
        {
            ...
            cbAbnBoiler.SelectedValue = -1;
            cbAbnHighNet.SelectedValue = -1;
...
//            cmbConsumer.SelectedIndex = -1;
        ...
        private bool CheckAbnObj()
        {
            ...
            if (actAbnObjReg != ActionAbnObjReg.Read)
            {
                ...
                // вышестоящая и котловая обязательные поля?
        ...
        private int InsertIntotAbnObjReg(int AbnObj)
        {
            ...
            //котловая и вышестоящая сетевая
            dr["idAbnBoiler"] = (cbAbnBoiler.SelectedValue == null) ? DBNull.Value : cbAbnBoiler.SelectedValue;
            dr["idAbnHighNet"] = (cbAbnHighNet.SelectedValue == null) ? DBNull.Value : cbAbnHighNet.SelectedValue;
...
//                dataGridViewImages.Rows.Add(new Bitmap(ImageCompress.LoadBitmap(ImageData), 120, 160), ImageCompress.LoadBitmap(ImageData), true, id);
        ...
        private void LoadLinkSchmAbn()
        {
            ...
            if (dgvLinkSchmAbnObj.RowCount > 0)
            {
                UpdateSchmLink();
            }
        ...
        {
            this.toolStripLinkSchmAbnObj.Enabled = true;
            this.dgvLinkSchmAbnObj.Enabled = true;
            dgvLinkSchmAbnObj.Columns["ShemeObjNameAll"].Visible = true;
        }
        ...
        //    BeginInvoke(new deligate1(ShowBottonSchmLink));
        //}

        private void ShowBottonSchmLink()
        {
            tsbLoadLinkSchm.Enabled = true;
            t.Abort();
        }

        private void UpdateSchmLink()
        {
            this.toolStripLinkSchmAbnObj.Enabled = false;
            this.dgvLinkSchmAbnObj.Enabled = false;
        ...
        private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            string str = "";
            foreach (DataRow row in dsAbn.vL_SchmAbn.Rows)
            {
                str = "";
                ...
                try
                {
                    ...
                    {
                        //центр питания
                        var listSchmObj = FormAbn.electricModel.GetCellCP((int)idSchmObj);
                        if (listSchmObj != null)
        ...
        private void tsbLoadLinkSchm_Click(object sender, EventArgs e)
        {
            if (dgvLinkSchmAbnObj.RowCount > 0)
            {
                UpdateSchmLink();
        ...
        private void btnNormal_Set_Click(object sender, EventArgs e)
        {
            // проверим есть ли нормальная привязка
            if (dgvLinkSchmAbnObj.Rows.Count == 0)
                return;

            foreach (DataGridViewRow dgvR in dgvLinkSchmAbnObj.Rows)
            {
                if (dgvR.Cells[Normal.Name].Value != DBNull.Value && (Boolean)dgvR.Cells[Normal.Name].Value)
                {
                    MessageBox.Show("Нормальная привязка уже имеется!");
                    return;
                }
            }
            ...
            try
            {
                ...
                string strCmd = "";
                strCmd = "sp_LegNormalSet";
                SqlCommand sqlCmd = new SqlCommand(strCmd, con.Connection);
                sqlCmd.CommandType = CommandType.StoredProcedure;

                int ID = Convert.ToInt32(dgvLinkSchmAbnObj.CurrentRow.Cells["idLinkSchmAbnDgvColumn"].Value);
                SqlParameter p1 = new SqlParameter("@id", ID);
                sqlCmd.Parameters.Add(p1);

                SqlParameter p2 = new SqlParameter("@Set", 1);
                sqlCmd.Parameters.Add(p2);
        ...
        private void btnNormal_No_Click(object sender, EventArgs e)
        {
            DataSql.SqlDataConnect con = new DataSql.SqlDataConnect();
            try
            {
                ...
                SqlParameter p2 = new SqlParameter("@Set", 2);
                sqlCmd.Parameters.Add(p2);
        ...
        private void GetAccessToBinding()
        {
            if (AccessToBinding < 0)
            {
                btnNormal_Set.Enabled = false;
                btnNormal_No.Enabled = false;
            }
        }
