...
--USE [GES]
------GO
------ убираем в колонках лишние пробелы, энтеры, переводы строк
...
abnLegal (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], KladrObjid, kladrObjName, idStreet, StreetName, House,
                    idCategory, CategoryName, idTariff, TarifName, TariffParentKey, TariffNameSocr, powerset, powerFact, sumKwt, idConsumerGroup, ConsumerGroupName, Consumer, consumerName, contact, idAbnNet, abnNet, abnServicing, CountMeter, CommentODS, cStatus, idMap) as
(
select  abnObj.idAbn, abnObj.codeAbonent, (cast (abnObj.codeAbonent as varchar)+isnull(' от '+CONVERT(varchar(10), dog.[DogDate],104)+' г.' , '')) as docAbn
          ...
          abnObj.id as idAbnObj, abnObj.nameObj, replace(replace(abnObj.address, char(10), ' '), char(13), ' ') as addres,
          abnObj.KladrObjid, abnObj.KladrObjName, 
           abnobj.idStreet, abnobj.StreetName, abnobj.house,
          -- objREg.Category, 
           cCategory.id,
           cCategory.name as CateGoryName,
           objReg.idTariff, isnull(cTariff.name, 'НН') AS TarifName,
           isnull(cTariff.ParentKey, ';SKUEE;Tariff;TariffLeg;') TariffParentKey, 
           cTariff.socrname as TariffNameSocr,
...
abnFL (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], KladrObjid, kladrObjName, idStreet, StreetName, House,
                    category, categoryName, idTariff, TarifName, TariffParentKey, TariffNameSocr, powerset, powerFact, sumKwt, idConsumerGroup, ConsumerGroupName, Consumer, consumerName, contact, idAbnNet, abnNet, abnServicing, CountMeter, CommentODS, cStatus, idMap) as
(
select  abnObj.idAbn, abnObj.codeAbonent, cast (abnObj.codeAbonent as varchar) as docAbn, 
          ...
          abnObj.id as idAbnObj, abnObj.nameObj, replace(replace(abnObj.address, char(10), ' '), char(13), ' ') as address,
          abnObj.KladrObjid, abnObj.KladrObjName,
          abnobj.idStreet, abnobj.StreetName, abnobj.House,
          --objREg.category,
          cCategory.id,
...
where abnObj.typeAbn in (206,253) and abnObj.isActive = 1 and abnObj.deleted = 0 
     ...
     and (statusAbn.idReason != 2340 or statusAbn.idReason is null)
          and (
               -- надо убрать 'нет допуска' (если данный дом находится в ГС)
          /*statusAbn.idReason = 740 and */ not exists (select abno.idMap
               from vAbnObj abno
               left join tAbnStatus AS stAbn ON stAbn.idAbn =  abno.idAbn AND stAbn.id =
                                    (SELECT     TOP (1) id
                                        FROM          dbo.tAbnStatus
                                        WHERE      (idAbn = abno.idAbn)
                                        ORDER BY DateChange DESC) 
               where  abno.idMap =   abnobj.idMap   and    stAbn.idReason = 2340 and abno.isActive = 1 and abno.deleted = 0
                    ) )
     ) ,
...
abnAll (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], kladrObjId, KladrObjName,/*idCity, City,*/ idStreet, StreetName, House,
                    /*idCategory*/Category, CategoryName, idTariff, TariffName, TariffParentKey, TariffNameSocr, powerset, powerFact, sumkwt, idConsumerGroup, ConsumerGroupName, Consumer, consumerName, contact, idAbnNet, abnNet/*nameAbnNet*/, abnServicing, CountMeter, CommentODS, cStatus, idMap) as
...
select
          ...
          , (select top 1 (vOwned.name) from tP_Passport AS pAssport 
                          LEFT JOIN tP_ObjectChar AS objChar ON pAssport.Id = objChar.idPassport  AND objChar.idObjChar in (174) -- код принадлежности подстанции
                         left join tP_ValueLists as vOwned  on vOwned.id = objChar.Value
                         where pAssport.isActive = 1 and pAssport.Deleted = 0 and pAssport.idObjList = isnull(cell.subid, isnull(bus.subid, sub.id))) Owning
          -- Ответсвенный работник
          , workerAbn.F + Case 
                                   When workerAbn.I is null or workerAbn.O is null then ''
                                   else  ' ' + SUBSTRING(isnull(workerAbn.I,''), 1, 1) + '. ' + SUBSTRING(isnull(workerAbn.O,''), 1, 1) + '.' 
                              end  Responsibility
          from 
               (select abn.*, 
            ...
            left join dbo.tR_KladrStreet AS street ON street.Id = map.Street
               ...
               --
               left join tAbn on tAbn.id = abn2.idAbn
               left join tR_Worker workerAbn on workerAbn.id = tAbn.idWorker
